                ...
                if (err) {
                    ...
                    user.save((err, userStored)=>{
                        if (err) {
                            ...
                            if (!userStored) {
                                res.status(404).send({message: "Error al agregar el usuario"});
                            } else {
                    ...
                    if(err){
                        ...
                        if(!userStored.active){
                            ...
                            res.status(200).send({
                                ...
                                refreshToken: jwt.createfreshToken(userStored)
                            });
                        }
                    }
                });
            ...
            if(!userData){
                ...
                if (req.files) {
                    ...
                    if(fileExt !== "png" && fileExt !== "jpg"){
                        res
                        .status(400)
                        .send({message:"La extension no es valida.(Extensiones acpetadas .png y .jpg)"
                        }); 
                    }else{
...
function activateUser(req, res){
    ...
    User.findByIdAndUpdate(id, {active},(err, userStored)=>{
        if (err) {
            ...
            if (!userStored) {
                res.status(404).send({message:"Nose ha encontrado el usuario"});
            }else{
              if (active === true) {
...
function addNewUser(req, res){
 ...
 function addClient(req, res){
     ...
     const {name, address, rfc, email, telefono, contacto}= req.body;
     client.name= name;
     client.address= address;
     client.rfc = rfc.toUpperCase();
     client.email= email;
     ...
     client.save((err, clientStored)=>{
         if(err){
             res.status(500).send({message:"Se produjo un error en el servidor"})
         }else{
             if(!clientStored){
